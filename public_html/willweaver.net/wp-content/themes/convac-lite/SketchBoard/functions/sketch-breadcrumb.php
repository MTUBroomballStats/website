...
class convac_lite_breadcrumb_class {
    ...
    function convac_lite_custom_breadcrumb() {
        ...
        global $post, $options;
        echo '<section class="cont_nav"><div class="cont_nav_inner"><p><a href="'. esc_url( home_url( '/' ) ) .'">' . __('Home', 'convac-lite') . '</a>';
    ...
    function convac_lite_simple_breadcrumb_case($der_post) {
        ...
        if (is_author()) {
            ...
            return __('Author', 'convac-lite') . ' : ' . $curauth->display_name;
        }

        if (is_tag()) {
            return __('Tag', 'convac-lite') . ' : ' . single_tag_title('', false);
        }

        if (is_search()) {
            return __('Search', 'convac-lite');
        }

        if (is_404()) {            
            return __('Error 404', 'convac-lite');       
        }
