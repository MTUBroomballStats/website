...
timeregex = re.compile('(1?[0-9]):([0-9]{2}) ([ap])m')
teamregex = re.compile('http://www.broomball.mtu.edu/team/view/([0-9]+)')
...
def timeParser(tstring):
	match = timeregex.search(tstring)
	prehour = int(match.group(1))
...
class SeasonParser(HTMLParser.HTMLParser):
	...
	def handle_data(self, data):
		...
		if self.state == 'ready_leagname':
			self.curLeague = self.foundLeague(data.strip())
			self.state = 'ready_exit_leagname'
			
		# Get conference name
		if self.state == 'ready_confname':
			self.curConf = self.foundConference(data.strip(), self.curLeague)
...
################################################################################
# GAME SCHEDULE PARSER                                                         #
################################################################################

class GameScheduleParser(HTMLParser.HTMLParser):
	...
	def __init__(self):
		...
		self.homeTeam = None
		self.awayTeam = None
		self.curRinkIndex = None
	
	def handle_starttag(self, tag, attrs):
		if tag == 'table':
			self.state = 'ready_rink_list'

		elif tag == 'tr':
			if self.state == 'ready_rink_list':
				self.state = 'ready_time_header'

			elif self.state == 'ready_time_row':
				self.state = 'ready_time'

		elif tag == 'td':
			if self.state == 'ready_time_header':
				self.state = 'in_time_header'

			elif self.state == 'ready_rink':
				self.state = 'in_rink'

			elif self.state == 'ready_time':
				self.state = 'in_time'

			elif self.state == 'ready_game':
				self.curRinkIndex += 1
				self.state = 'ready_away'

		elif tag == 'a':
			if self.state == 'ready_away':
				for attr in attrs:
					if attr[0] == 'href':
						match = teamregex.search(attr[1].strip())
						self.awayTeam = int(match.group(1))
						break
				self.state = 'ready_exit_away'

			elif self.state == 'ready_home':
				for attr in attrs:
					if attr[0] == 'href':
						match = teamregex.search(attr[1].strip())
						self.homeTeam = int(match.group(1))
						break
				self.state = 'ready_exit_home'
	
	def handle_endtag(self, tag):
		if tag == 'table':
			if self.state == 'ready_time_row':
				self.state = None

		elif tag == 'td':
			if self.state == 'ready_exit_time_header':
				self.state = 'ready_rink'

			elif self.state == 'ready_exit_rink':
				self.state = 'ready_rink'

			elif self.state == 'ready_exit_time':
				self.curRinkIndex = -1
				self.state = 'ready_game'

			elif self.state == 'ready_exit_game':
				self.foundGame(self.homeTeam, self.awayTeam, self.curTime, self.rinkList[self.curRinkIndex])
				self.state = 'ready_game'

		elif tag == 'tr':
			if self.state == 'ready_rink':
				self.state = 'ready_time_row'

			elif self.state == 'ready_game':
				self.state = 'ready_time_row'

		elif tag == 'a':
			if self.state == 'ready_exit_away':
				self.state = 'ready_home'

			elif self.state == 'ready_exit_home':
				self.state = 'ready_exit_game'
	
	def handle_data(self, data):
		if self.state == 'in_time_header':
			self.state = 'ready_exit_time_header'

		if self.state == 'in_rink':
			self.rinkList.append(data.strip())
			self.state = 'ready_exit_rink'

		if self.state == 'in_time':
			self.curTime = timeParser(data.strip())
			self.state = 'ready_exit_time'
	
	def foundGame(self, home, away, time, location):
		pass
...
class PlayoffBracketParser(HTMLParser.HTMLParser):
	def __init__(self):
		self.state = None
		self.teams = dict()
		self.round = None
		...
		self.roundRegex = re.compile('bbox1\sround-([0-9]+)-box')
		self.blinelRegex = re.compile('blinel\sround-([0-9]+)-box')
		self.blinerRegex = re.compile('bliner\sround-([0-9]+)-box')
	
	def handle_starttag(self, tag, attrs):
		if tag == 'div':
			if self.state is None:
				for attr in attrs:
					if attr[0] == 'class' and attr[1] == 'bcontainer':
						self.state = 'ready_round'
						break
			
			elif self.state == 'ready_round':
				for attr in attrs:
					if attr[0] == 'class' and attr[1] == 'round':
						self.state = 'ready_game'
						break
				...
				for attr in attrs:
					if attr[0] == 'class':
						match = self.roundRegex.search(attr[1].strip())
						if match is not None:
							self.round = int(match.group(1))
							self.state = 'ready_innerb'
							break
							
						match = self.blineRegex.search(attr[1].strip())
						if match is not None:
							self.round = int(match.group(1))
							self.state = 'ready_innerb'
							break
						
			elif self.state == 'ready_innerb':
				for attr in attrs:
					if attr[0] == 'class' and attr[1] == 'innerb':
						self.state = 'ready_homet'
						break
			
			elif self.state == 'ready_homet':
				for attr in attrs:
					if attr[0] == 'class' and attr[1] == 'homet':
						self.state = 'ready_home'
						break
						
			elif self.state == 'ready_awayt':
				for attr in attrs:
					if attr[0] == 'class' and attr[1] == 'awayt':
						self.state = 'ready_away'
						break
						
		elif tag == 'b':
			if self.state == 'ready_home':
				self.winner = self.home
				self.state = 'ready_home'
			
			elif self.state == 'ready_away':
	...
	def handle_endtag(self, tag):
		if tag == 'div':
			if self.state == 'ready_exit_homet':
				self.state = 'ready_awayt'
				
			elif self.state == 'ready_exit_awayt':
				self.state = 'ready_exit_innerb'
				
			elif self.state == 'ready_exit_innerb':
				if self.home in self.teams:
					if self.round > self.teams[self.home]:
						self.teams[self.home] = self.round
				
				if self.away in self.teams:
					if self.round > self.teams[self.away]:
						self.teams[self.away] = self.round
						
				self.state = 'rady_exit_game'
				
			elif self.state == 'ready_exit_game':
				self.state = 'ready_game'
				
		elif tag == 'a':
			if self.state == 'ready_exit_home':
				self.state = 'ready_exit_homet'
				
			elif self.state == 'ready_exit_away':
				self.state = 'ready_exit_awayt'
	
	def handle_data(self, data):
		if self.state == 'ready_home':
			self.state = 'ready_exit_innerb'
	
	def foundTeam(self, team):
		pass
